	...
	if (isset($_SESSION['idusuario'])) {
		...
		switch ($_GET['action']) {
			...
			case 'readOnes':
				...
				break;
			case 'searchOneQuestion':
				$_POST = $paciente->validateForm($_POST);
				if ($_POST['nombre_pacientepr'] != '') {
					if ($result['dataset'] = $paciente->searchOneQuestion($_POST['nombre_pacientepr'])) {
						$result['status'] = 1;
						$rows = count($result['dataset']);
						if ($rows > 1) {
							$result['message'] = 'Se encontraron ' . $rows . ' coincidencias';
			...
			case 'readOneQuestion':
				if ($paciente->setId($_POST['id_pacientepr'])) {
					$result['dataset'] = $paciente->readOneQuestion();
					$result['status'] = 1;
				} else {
					$result['exception'] = 'Paciente sin datos';
				}
				break;
			case 'readOneQuestion1':
				if ($paciente->setId($_POST['id_pacientesU'])) {
					if ($result['dataset'] = $paciente->readOneQuestion1()) {
						$result['status'] = 1;
					} else {
						if (Database::getException()) {
							$result['exception'] = Database::getException();
				...
				break;
			case 'updateRowAnswers':
				$_POST = $paciente->validateForm($_POST);
				if ($paciente->setId($_POST['id_pacientesU'])) {
					if ($data = $paciente->readOneQuestion1()) {
						if ($paciente->setRespuesta($_POST['notas'])) {
							if ($paciente->updateRowRespuesta()) {
								$result['status'] = 1;
								$result['message'] = 'Datos Ingresados correctamente';
							} else {
								$result['exception'] = Database::getException();
