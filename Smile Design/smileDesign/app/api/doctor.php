    ...
    if (isset($_SESSION['idusuario'])) {
        ...
        switch ($_GET['action']) {
                                                ...
                                                if ($doctores->setEstado($_POST['estado_doctor'])) {
                                                    if (is_uploaded_file($_FILES['foto_doctor']['tmp_name'])) {
                                                        ...
                                                        if ($doctores->updateRow($data['fotodoctor'])) {
                                                            $result['status'] = 1;
                                                            if ($doctores->saveFile($_FILES['foto_doctor'], $doctores->getRuta(), $doctores->getFoto())) {
                                                                $result['message'] = 'La Información del Doctor fue modificada exitosamente';
                                                            } else {
                                                                $result['message'] = 'La Información del Doctor se guardo, pero la imagen no fue guardada';
                                                            }
                                                        } else {
            ...
            case 'readOnes':
                ...
                break;
            case 'readTopDoctores':
                if ($result['dataset'] = $doctores->readTopDoctores()) {
                    $result['status'] = 1;
                } else {
                    if (Database::getException()) {
                        $result['exception'] = Database::getException();
